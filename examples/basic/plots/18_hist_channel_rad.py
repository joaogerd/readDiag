...
"""
Uso:
  python 18_hist_channel.py <diag_rad_path> [channel] [param] [bins]
"""
if __name__ == "__main__":
    fpath = arg_or_default(1, PROJECT_ROOT / "data/diag_amsua_n19_01.2024013018")
    ch    = int(arg_or_default(2, 5))
    param = arg_or_default(3, "omf")
    bins  = int(arg_or_default(4, 50))
    ax = diagPlotter(open_diag(fpath)).plot_hist_channel(ch, param=param, bins=bins)
    out = ensure_outdir() / f"18_hist_channel_{ch}_{param}.png"
